		...
		mobile_iframe: function() {
			...
			if (t && !n) {
				e(window).on("message", function(e) {
					...
					var i = r.replace(/^([^\/]+\/\/)(assets\.)(\.*)/, "$1$3").replace(/^([^\/]+\/\/)(tumblr\.com)/i, "$1www.tumblr.com");
					var s = e.originalEvent.data.split(";");
					if (s[0] === "tumblr_controls" && (s[1] === "closed" || s[1] === "open")) {
						t.contentWindow.postMessage("set_variation;corner_buttons", r);
						if (r !== i) {
							t.contentWindow.postMessage("set_variation;corner_buttons", i)
						}
						n = true
					}
				})
